...
async function calculateAndUpdateGovernanceScore() {
  try {
    ...
    const policyAckPercentage = totalAcks > 0 ? (acknowledgedCount / totalAcks) * 100 : 100;
    ...
    if (totalIssues > 0) {
      ...
      complianceResolutionRate = Math.max(0, (scoreWeight / totalIssues) * 100);
    }

    // 4. Aggregate Governance Score
    const governanceScore = Math.round(
      (policyAckPercentage * 0.4) +
      (auditCompletionRate * 0.3) +
      (complianceResolutionRate * 0.3)
    );

    // 5. Update all Department documents
    const departments = await Department.find();
    for (const dept of departments) {
      dept.governanceScore = governanceScore;

      // Recompute Total ESG Score (default: Environmental 40% / Social 30% / Governance 30%)
      const envScore = dept.environmentalScore || 0;
      const socScore = dept.socialScore || 0;
      dept.totalESGScore = Math.round((envScore * 0.4) + (socScore * 0.3) + (governanceScore * 0.3));
